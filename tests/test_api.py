...
class TestTypes:
    ...
    def test_decimal_encode(self):
        assert DataClassWithDecimal(Decimal(self.decimal_s)).to_json() == self.dc_decimal_json

    def test_decimal_decode(self):
        assert DataClassWithDecimal.from_json(self.dc_decimal_json) == DataClassWithDecimal(Decimal(self.decimal_s))
...
class TestGenericExtendedTypes:
    def test_optional_datetime(self):
        ...
        assert DataClassWithOptionalDatetime.from_json(dc.to_json()) == dc

    def test_optional_decimal(self):
        dc = DataClassWithOptionalDecimal(Decimal("12345.12345"))
        assert DataClassWithOptionalDecimal.from_json(dc.to_json()) == dc

    def test_optional_uuid(self):
        dc = DataClassWithOptionalUuid(UUID("d1d61dd7-c036-47d3-a6ed-91cc2e885fc8"))
        assert DataClassWithOptionalUuid.from_json(dc.to_json()) == dc
...
class TestDictDecode:
    ...
    def test_decimal_decode(self):
        assert DataClassWithDecimal.from_dict(self.dc_decimal_json) == DataClassWithDecimal(Decimal(self.decimal_s))

    def test_uuid_decode(self):
        assert DataClassWithUuid.from_dict(self.dc_uuid_json) == DataClassWithUuid(UUID(self.uuid_s))
...
class TestNewType:
    ...
    def test_new_type_encode(self):
        assert DataClassWithNewType(UUIDWrapper(UUID(self.new_type_s))).to_json() == self.dc_new_type_json

    @pytest.mark.skipif(sys.version_info >= (3, 10), reason="newtype decode breaks in 3.10")
    def test_new_type_decode(self):
        assert DataClassWithNewType.from_json(self.dc_new_type_json) == DataClassWithNewType(
            UUIDWrapper(UUID(self.new_type_s))
        )

    def test_nested_new_type_encode(self):
        assert (
            DataClassWithNestedNewType(UUIDWrapperWrapper(UUIDWrapper(UUID(self.new_type_s)))).to_json()
            == self.dc_new_type_json
        )

    @pytest.mark.skipif(sys.version_info >= (3, 10), reason="newtype decode breaks in 3.10")
    def test_nested_new_type_decode(self):
        assert DataClassWithNestedNewType.from_json(self.dc_new_type_json) == DataClassWithNestedNewType(
...
class TestInferMissing:
    ...
    def test_infer_missing_is_recursive(self):
        actual = DataClassWithOptionalNested.from_json('{"x": {}}', infer_missing=True)
    ...
    def test_infer_missing_terminates_at_first_missing(self):
        actual = DataClassWithOptionalNested.from_json('{"x": null}', infer_missing=True)
...
class TestWarnings:
    def test_warns_when_nonoptional_field_is_missing_with_infer_missing(self):
        with pytest.warns(RuntimeWarning, match="Missing value"):
            actual = DataClassWithDataClass.from_json('{"dc_with_list": {}}', infer_missing=True)
    ...
    def test_warns_when_required_field_is_none(self):
        with pytest.warns(RuntimeWarning, match="`NoneType` object"):
            assert DataClassWithDataClass.from_json('{"dc_with_list": null}') == DataClassWithDataClass(None)
...
class TestSchema:
    def test_dumps_many(self):
        actual = DataClassWithList.schema().dumps([DataClassWithList([1])], many=True)
        json_s = '[{"xs": [1]}]'
        assert actual == json_s

    def test_dumps_many_nested(self):
        dumped = DataClassWithDataClass.schema().dumps([DataClassWithDataClass(DataClassWithList([1]))], many=True)
        json_s = '[{"dc_with_list": {"xs": [1]}}]'
        assert dumped == json_s

    def test_loads_many(self):
        json_s = '[{"xs": [1]}]'
        assert DataClassWithList.schema().loads(json_s, many=True) == [DataClassWithList([1])]
    ...
    def test_loads_default(self):
        assert DataClassIntImmutableDefault.schema().loads("{}") == DataClassIntImmutableDefault()
        assert DataClassBoolImmutableDefault.schema().loads("{}") == DataClassBoolImmutableDefault()

    def test_loads_default_many(self):
        assert DataClassIntImmutableDefault.schema().loads("[{}]", many=True) == [DataClassIntImmutableDefault()]
        assert DataClassBoolImmutableDefault.schema().loads("[{}]", many=True) == [DataClassBoolImmutableDefault()]
    ...
    def test_loads_infer_missing(self):
        assert DataClassWithOptional.schema(infer_missing=True).loads("[{}]", many=True) == [
            DataClassWithOptional(None)
        ]

    def test_loads_infer_missing_nested(self):
        assert DataClassWithOptionalNested.schema(infer_missing=True).loads("[{}]", many=True) == [
