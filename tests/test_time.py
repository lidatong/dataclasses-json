...
class TestTime:
    ...
    def test_datetime_override_decode(self):
        assert DataClassWithIsoDatetime.from_json(self.dc_iso_json) == self.dc_iso

    def test_datetime_schema_encode(self):
        assert DataClassWithDatetime.schema().dumps(self.dc_ts) == self.dc_ts_json

    def test_datetime_schema_decode(self):
        assert DataClassWithDatetime.schema().loads(self.dc_ts_json) == self.dc_ts

    @pytest.mark.skipif(sys.version_info < (3, 7), reason="requires python3.7")
    def test_datetime_override_schema_encode(self):
        assert DataClassWithIsoDatetime.schema().dumps(self.dc_iso) == self.dc_iso_json
    ...
    def test_datetime_override_schema_decode(self):
        ...
        # #955
        iso.created_at = iso.created_at.replace(microsecond=456753, tzinfo=self.tz)
        assert iso == self.dc_iso

    @pytest.mark.skipif(sys.version_info < (3, 7), reason="requires python3.7")
    def test_datetime_custom_iso_fieldoverride_schema_encode(self):
        assert DataClassWithCustomIsoDatetime.schema().dumps(self.dc_iso) == self.dc_iso_json
